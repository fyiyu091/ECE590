...
namespace elma {
    ...
    // Yiyu->event_handlers with the higher priority is running before the event handler with the lower priority
    // Sense a event coming but finish the higher priority event firstly
    // Need to find where the event is being running...
    Manager& Manager::watch(std::string event_name, std::function<void(Event&)> handler, int priority) {
        // decide the position to arrange the new added handler
        int t_position;
        while (name_vec.empty() == 1)
        {
            name_vec.push_back(event_name);
        }

        if (name_vec[name_vec.size()-1] != event_name)
        {
                p_vec.clear();
                name_vec.push_back(event_name);
        }
        ...
        {
            t_position = 0;
            p_vec.push_back(priority);
        }
        else if (priority >= p_vec[0])
        {
            t_position = 0;
            p_vec.insert(p_vec.begin(),priority);
        }
        else if (priority <= p_vec[p_vec.size()-1])
        {
            t_position = p_vec.size();
            p_vec.push_back(priority);
        }
        else 
        {
            for (int i=0; i<p_vec.size(); i++)
            {
                ...
                {
                   t_position = i;
                   p_vec.insert(p_vec.begin()+t_position,priority);
                   break;
                }
            ...
            for (int i=0; i<p_vec.size(); i++)
            {
                if (priority < p_vec[i] && priority > p_vec[i+1])
                {
                   t_position = i+1;
                   p_vec.insert(p_vec.begin()+t_position,priority);
                }
        ...
        //std::sort(p_vec.begin(),p_vec.end(),[](int a, int b){return a>b;});
        //std::cout << "the t_ position is " << t_position << '\n';
        // arrange the new added hander to the position    
        event_handlers[event_name].push_back(handler);
        event_handlers[event_name].insert(event_handlers[event_name].begin()+t_position, event_handlers[event_name][event_handlers[event_name].size()-1]);
        event_handlers[event_name].erase(event_handlers[event_name].end()-1);
