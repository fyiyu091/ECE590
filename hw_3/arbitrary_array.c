...
char * ArbitraryArray_to_string(const ArbitraryArray * a, char * (*element_to_string) (void *) )
{
    int i, j;   
    int num = 0;
    char * results = (char *) calloc(40, sizeof(char));
    for (i = 0; i < ArbitraryArray_size(a); i++)
    {
        for (j = 0; j <= (a->element_size)/2; j++)
        {
            results[i*((a->element_size)/2)+j+num] = element_to_string (ArbitraryArray_get_ptr(a, i))[j];
        }
        if (i != ArbitraryArray_size(a) - 1)
        {
        results[i*((a->element_size)/2)+j+num] = ',';
        num=num+2;
        }
    }
    return results;
