...
protected:
    ...
    void print_grade() {
        ...
        GTEST_COUT_GRADE << "Question breakdown: ";
        print_vector(q);
    }
...
TEST_P(ReadTests, ReadRandomBrokenCSV) {
    ...
    if (r > 0) {
        ...
        row.push_back("1.0");
        string path = save_csv(s, "broken.csv");
...
TEST_P(ReadTestsWhiteSpace, ReadRandomCSVWithSpaces) {
    ...
    string path = save_csv(s, "withspaces.csv");
}
...
TEST_P(WriteTests, WriteRandomCSV) {
    ...
    ASSERT_NO_DEATH(write_matrix_csv(m, "WriteTests.csv"), ".*");

    write_matrix_csv(m, "WriteTests.csv");
    }
